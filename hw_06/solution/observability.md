...
собираются стандартными exporter'ами — не проходят тест «что предпринять» лучше, чем 16
метрик ниже.
...
| 10 | ClickHouse | USE Saturation | parts pending merge + lag реплики шарда при `insert_quorum=2` | `system.parts` / `system.replicas` | мелкие parts — антипаттерн INSERT и причина роста p95 Query. Lag одной реплики при трёх репликах и `insert_quorum=2` запись не останавливает; отказ кворума — когда жива только одна реплика шарда ([`reliability.md` §2](reliability.md#2-репликация-критичных-хранилищ)) |
...
| 15 | Query API (алерты) | Бизнес | возраст последней успешной проверки правил: `now − max(last_checked_at)` | gauge по `alert_state` ([ADR-0006](arc42/adr/0006-threshold-alerts-webhook.md)); нет активных правил — gauge не алертим | расписание 15 мин. Возраст > 30 мин (2× тика) — правила никто не пересчитал: lease не захвачен, оба инстанса Query API недоступны или тик падает до записи `last_checked_at`. p95 дашборда (#6) это не показывает: владелец может читать воронку и не получить webhook |
| 16 | Query API (алерты) | Бизнес | доля webhook, исчерпавших retry без 2xx | `exhausted / attempts` за тик; знаменатель 0 — доля не считается | доставка at-least-once молча умирает на мёртвом `webhookUrl`. Рост доли — разбирать статус в `alert_state` и аудит, не перезапускать весь Query API |

## 2. Алерты

Девять алертов сформулированы как пользовательский или платформенный симптом (SDK не может
сдать батч; принятые события не попадут в SoT; дашборд не отвечает за 2 с; правило давно
не проверялось; webhook не доставлен), а не как загрузка CPU. Для одного инцидента — один
алерт: freshness (#13) и CPU Writer на отдельные правила не вынесены, их закрывают #3 и
метрика #8. Каждый опирается на строку из [п.1](#1-ключевые-метрики) и на механизм из
[`reliability.md`](reliability.md).

Проверка alert-правил живёт на Query API — Tier 2, доступность 99.5%
([`requirements.md` §1.2](requirements.md#12-нефункциональные-требования),
[`reliability.md` §1](reliability.md#1-rto--rpo-по-сервисам)). Тир не повышаем: простой
проверки не теряет события. Непроверенное правило и неуспешный webhook этим SLO не видны —
их закрывают #15 и #16.

Порог ошибок Ingest (0.5% / 3 мин) следует из SLO приёма 99.9%, а не из доступности Query API
...
```
Название:        Правила алертов давно не проверялись
Условие/порог:   now − max(last_checked_at) > 30 мин при хотя бы одном
                 активном правиле (метрика #15). 30 мин = 2 × расписание 15 мин
Severity:        warning
Что означает:    порог мог быть нарушен, а webhook не ушёл. Владелец этого
                 не видит: дашборд по-прежнему отвечает. Query API — 99.5%,
                 простой проверки события не теряет
Вероятная причина: lease не захвачен, оба инстанса Query API недоступны,
                 тик падает до UPDATE last_checked_at, Postgres App & Config
                 не принимает internal-вызов
Реакция:         есть ли holder в alert_scheduler_lease и не протух ли
                 leased_until → логи тика Query API → доступность App & Config.
                 Не поднимать Query API в Tier 0
Кому:            on-call backend (Query API)
```

```
Название:        Webhook алерта не доставлен
Условие/порог:   доля webhook, исчерпавших retry без 2xx, > 0 за тик
                 (метрика #16; знаменатель — попытки доставки за 15 мин)
Severity:        warning
Что означает:    правило сработало, владелец сигнал не получил. Повторный
                 POST на ту же просадку cooldown уже мог запретить
Вероятная причина: webhookUrl недоступен, таймаут, отказ подписи на стороне
                 получателя
Реакция:         last_webhook_status и аудит (alertId, status, без секрета
                 и без тела) → не крутить retry вручную сверх бюджета →
                 владелец чинит URL или ротирует секрет. Правило не удалять
Кому:            on-call backend (Query API)
```
...
| Правила давно не проверялись | возраст `last_checked_at` > 30 мин | warning | порог мог нарушиться без webhook | lease, тик Query API, App & Config |
| Webhook не доставлен | доля исчерпанных retry без 2xx > 0 / тик | warning | правило сработало, владелец не получил сигнал | статус и аудит, не удалять правило |
...
**Sampling.** Пик Ingest — 1 111 HTTP ([`sizing.md` §1](sizing.md#1-расчёт-rps)); 100% успешных батчей
